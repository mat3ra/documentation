...
This feature only works for uploading files to the [Login Home]({{ resources_url }}/infrastructure/login/directories/) directory, but not to any of its sub-folders. In case of necessity, the use of the [SCP protocol](../ssh.md#transfer-files-with-scp) is recommended for transferring files, which works under all circumstances.  
