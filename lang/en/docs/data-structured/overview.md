...
We store the structured data in the MongoDB database [^1]. The database is highly available, ie. has multiple equivalent sources at any time, and is backed up daily. Readers are referred to the [Collections]({{ reference_url }}/accounts/collections/) and [Infrastructure]({{ dev_url }}/infrastructure/overview/) sections to find out more about how individual items are stored and organized for access.  
