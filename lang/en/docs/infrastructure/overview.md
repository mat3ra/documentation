...
We additionally support platform-level access via [advanced connection methods]({{ interface_url }}/remote-connection/overview/), as an alternative to the main Web Interface. Platform-level access methods are intended for expert users and also offer the ability to submit [simulation jobs]({{ guide_url }}/jobs-cli/overview/) to the [computing clusters](clusters/overview.md), for example.
...
- Blue lines indicate [remote connection methods]({{ interface_url }}/remote-connection/overview/) to our platform.
...
## 2. [Remote Desktop]({{ interface_url }}/remote-connection/remote-desktop/)

A remote desktop environment is offered for connecting to the platform and accessing the relevant data stored in its different nodes. This is explained in details [here]({{ interface_url }}/remote-connection/remote-desktop/).
...
## 4. [SSH]({{ interface_url }}/remote-connection/ssh/)

The [Command Line Interface]({{ guide_url }}/cli/overview/) can also be accessed via an external SSH client instead of the Web Terminal. [This page]({{ interface_url }}/remote-connection/ssh/) outlines the instructions on how to do so.
...
## 6. [Web Terminal]({{ interface_url }}/remote-connection/web-terminal/)

Alternatively to the aforementioned Remote Desktop connection method, the platform can also be accessed via the Command Line Interface described [here]({{ guide_url }}/cli/overview/). We provide a [Web Terminal]({{ interface_url }}/remote-connection/web-terminal/) utility for logging in via Command Line directly from the Web Interface.
