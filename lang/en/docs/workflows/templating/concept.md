...
The templates (input files) are rendered in two places, on [web interface]({{ guide_url }}/ui/overview/) when workflows (or jobs) are designed and on [computing clusters]({{ dev_url }}/infrastructure/clusters/overview/) when the job is executed, henceforth referred to as **Design-time Rendering** and **Runtime Rendering** respectively. The is necessary as some templates require data which is only available at runtime (e.g. `outdir` in Quantum Espresso PWScf input file)
