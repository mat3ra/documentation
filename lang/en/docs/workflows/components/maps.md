...
In practice, when a new map is added to a workflow [flowchart]({{ interface_url }}/workflow-designer/sidebar/), via the actions introduced [in this page]({{ interface_url }}/workflow-designer/subworkflow-editor/actions-menu/#Add Subworkflows), an entire new workflow will be created and contained within the original parent workflow. The typical appearance of this "workflow inside another workflow" map unit is shown in the example below.
